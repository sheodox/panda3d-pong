...
class MyApp(ShowBase):
    def __init__(self):
        ...
        self.camera.set_pos(0, 0, 90)
        self.camera.set_hpr(0, -90, 0)
        lens = OrthographicLens()
        lens.set_film_size(40 * win_aspect, 40)
        self.cam.node().setLens(lens)
        self.gc = GameControls(self, self.move_player, mouse_magnitude=3)
        ...
        self.cTrav.add_collider(cnodePath, self.coll_hand_event)

        # ball movement
        self.ball_v = LVector3f(-.7, 1, 0)
        self.ball_speed_scale = 0.15
        self.task_mgr.add(self.ball_move_task, 'ball-move')
        ...
        self.player_side_cnode = self.render.attachNewNode(CollisionNode('pplane'))
        ...
        self.ai_side_cnode = self.render.attachNewNode(CollisionNode('aiplane'))
        ...
        for np in [pp, aip, self.bottom_boundary, self.top_boundary]:
            ...
            self.cTrav.add_collider(cnodePath, self.coll_hand_event)
        self.accept('into-wall', self.ball_collision)
        self.accept('into-pplane', self.player_side_goal)
        self.accept('into-aiplane', self.ai_side_goal)
    ...
    def ball_collision(self, entry):
        norm = entry.get_surface_normal(self.render) * -1
    ...
    def load_and_render(self, model_path, copy=False):
        if copy:
            model = self.loader.loadModelCopy(model_path)
        else:
